...
const UI = (() => {
  ...
  function initAppLoader() {
    ...
    if (!loader) return;
    const MIN_DISPLAY_MS = 7000; // splash screen tampil tetap 7 detik
    const MAX_WAIT_MS = 7000;    // jaring pengaman: paksa hilang walau event 'load' tidak pernah tembak
    const startedAt = performance.now();

    // Teks status yang berganti selama splash screen tampil
    const statusEl = document.getElementById('loader-status');
    const statusMessages = [
      'Menyiapkan katalog produk...',
      'Memuat data cabang toko...',
      'Menyusun promo hari ini...',
      'Hampir siap...'
    ];
